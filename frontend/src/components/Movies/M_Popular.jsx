...
class M_Popular extends React.Component{
    constructor(props){
        super(props);
    ...
    componentDidMount(){
        fetch(`${ API_URL}movie/popular?api_key=${API_KEY}&language=en-US&page=${this.state.currentPage}`)
    ...
    handleClick(){
        ...
        fetch(`${API_URL}movie/popular?api_key=${API_KEY}&language=en-US&page=${this.state.currentPage+1}`)
        .then((response)=>response.json())
        .then((response)=>this.setState({
    ...
    render(){
        ...
        return(
            <div className="container">
                <h2 style={{fontWeight:"700", fontSize:"2.5em" ,marginTop:"1rem"}}>Popular Movies</h2>
