...
class M_Cast_Crew extends React.Component{
        constructor(props){
            super(props);
    ...
    componentDidMount(){
        fetch(`${API_URL}movie/${this.state.movie_id}/credits?api_key=${API_KEY}&language=en-US`)
 ...
 render(){
    ...
    let Cast = this.state.cast.map((item)=>{
        ...
        return(       
           <div className="col-sm-2" key ={item.cast_id}>
    ...
    return (
        ...
        <div style={{overflowX :"scroll",overflowY:"hidden", display :"flex" }}>{ Cast }</div>
